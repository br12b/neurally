...
import { ArrowRight, ScanLine, AlertCircle, PlayCircle } from 'lucide-react';
...
export default function LoginScreen({ onLogin }: LoginScreenProps) {
  ...
  const handleGoogleLogin = async () => {
    ...
    if (window.location.protocol === 'file:') {
        setErrorMsg("KRİTİK HATA: Google Girişi dosya sisteminden (file://) çalışmaz. Lütfen localhost kullanın.");
        setIsLoading(false);
        return;
    }
    ...
    try {
        ...
        if (isMobile) {
            console.log("Mobile environment detected. Initiating Redirect...");
            // Set flag for App.tsx to catch on return
        ...
        if (error.code === 'auth/unauthorized-domain') friendlyMessage = "Yetkisiz Alan Adı: Firebase Console'dan domaini ekleyin.";
        else if (error.code === 'auth/operation-not-allowed') friendlyMessage = "Google Girişi Firebase'de aktif değil.";
      ...
      >
        ...
        <div className="w-full max-w-xs space-y-4">
           {errorMsg && (
               ...
               >
                   <div className="flex items-center gap-2 font-bold text-red-300"><AlertCircle className="w-4 h-4" /> HATA</div>
                   ...
                   >
                     <PlayCircle className="w-3 h-3" /> DEMO MODU
                   </button>
               </motion.div>
           )}
